...
def histogram(df_new):
	...
	py.plot(fig, filename = '6-42 Histogram (Whole)', auto_open=True)
...
def main():
    ...
    df['first_mean'] = df['first'].mean()
    df['second_mean'] = df['second'].mean()
    df['third_mean'] = df['third'].mean()
    df['fourth_mean'] = df['fourth'].mean()
    df['fifth_mean'] = df['fifth'].mean()
    df['sixth_mean'] = df['sixth'].mean()
